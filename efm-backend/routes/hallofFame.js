...
router.get('/hall-of-fame', async (req, res) => {
    try {
        console.log("🔌 [HOF ROUTE] Fetching Hall of Fame documents from MongoDB...");
...
router.get('/hof-debug', async (req, res) => {
    try {
        ...
        const recentLeagues = await require('../models/League').find().sort({ updatedAt: -1 }).limit(3);
...
router.post('/force-complete/:leagueId', async (req, res) => {
    try {
        ...
        const league = await League.findById(leagueId).populate('players', 'username');
        ...
        const fixtures = await Fixture.find({ leagueId: league._id, status: 'confirmed' });
        
        const statsMap = {};
        league.players.forEach(p => {
            statsMap[p._id.toString()] = { userId: p._id, username: p.username, teamName: 'Club Owner', points: 0, gd: 0 };
        });
        ...
        if (sortedStandings.length === 0) {
            return res.status(400).json({ success: false, error: "No player statistics could be computed for this league." });
        }
        ...
        const hofRecord = await HallOfFame.create({
            ...
            podium: {
                winner: sortedStandings[0] ? { userId: sortedStandings[0].userId.toString(), username: sortedStandings[0].username, teamName: sortedStandings[0].teamName } : null,
                runnerUp: sortedStandings[1] ? { userId: sortedStandings[1].userId.toString(), username: sortedStandings[1].username, teamName: sortedStandings[1].teamName } : null,
                thirdPlace: sortedStandings[2] ? { userId: sortedStandings[2].userId.toString(), username: sortedStandings[2].username, teamName: sortedStandings[2].teamName } : null
            }
        });
