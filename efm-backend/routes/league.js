...
const pushAndEmitNotification = async (req, userIds, notificationPayload) => {
  try {
    ...
    if (io) {
      stringIds.forEach((id) => {
        ...
        User.findById(id)
          ...
          .catch((err) =>
            console.error(
              `Socket broadcast pipeline failure for user ${id}:`,
              err,
            ),
          );
      });
    ...
    console.error(
      "Failed executing pushAndEmitNotification pipeline helper:",
      error,
    );
  }
};
...
router.get("/recruiting", async (req, res) => {
  try {
    const leagues = await League.find({ status: "recruiting" }).select(
      "name maxStrengthLimit capacity players rounds rules createdAt",
    );
...
router.get("/active", async (req, res) => {
  try {
    const leagues = await League.find({ status: "active" }).select(
      "name maxStrengthLimit capacity players currentMatchday rounds rules createdAt",
    );
...
router.get("/all", async (req, res) => {
  try {
    ...
    const leagues = await League.find({}).select(
      "name maxStrengthLimit capacity players status currentMatchday rounds tournamentFormat groupStageCount rules createdAt",
    );
...
router.get("/my-leagues/:userId", async (req, res) => {
  try {
    const userLeagues = await League.find({
      ...
      "name maxStrengthLimit capacity players status currentMatchday rounds rules createdAt",
    );
...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const userLeagues = await League.find({
      ...
      "name status capacity slotsFilled maxStrengthLimit currentMatchday rounds rules tournamentFormat",
    ); // 🚀 FIXED: Added tournamentFormat here
...
router.post("/:id/join", async (req, res) => {
  try {
    ...
    if (league.status !== "recruiting")
      return res
        .status(400)
        .json({ success: false, error: "Registration locked." });
    ...
    if (!player)
      return res
        .status(404)
        .json({ success: false, error: "Player profile not found." });
    if (player.teamStrength > league.maxStrengthLimit) {
      return res
        .status(400)
        .json({ success: false, error: "Team strength exceeds limit." });
    }
    if (league.players.length >= league.capacity)
      return res.status(400).json({ success: false, error: "League full!" });
    ...
    if (triggerScheduleGeneration) {
      ...
      if (formatType === "knockout") {
        ...
        formatType.includes("group")
      ) {
        const parsedGroupCount = parseInt(league.groupStageCount, 10);
        const finalGroupsCount =
          parsedGroupCount && parsedGroupCount > 0 ? parsedGroupCount : 4;
        ...
        schedulePlan = generateClassicLeague(
          league.players,
          league.rounds || 1,
        );
      }
      ...
      const finalizedFixtures = schedulePlan.map((match) => ({
        ...
        stageType: formatType.includes("group")
          ? "group_stage"
          : "knockout_stage",
      }));
      ...
      await pushAndEmitNotification(
        req,
        league.players,
        leagueLaunchNotification,
      );
    }
...
router.post("/fixtures/:fixtureId/submit", async (req, res) => {
  try {
    ...
    if (!fixture) {
      return res
        .status(404)
        .json({ success: false, error: "Fixture not found." });
    }
    ...
    if (fixture.status === "confirmed") {
      return res
        ...
        .json({
          success: false,
          error: "This match result has already been finalized.",
        });
    }
    ...
    if (
      fixture.playerASubmittedScore !== null &&
      fixture.playerBSubmittedScore !== null
    ) {
      const doesPlayerAAlign =
        fixture.playerASubmittedScore === fixture.playerAScore;
      const doesPlayerBAlign =
        fixture.playerBSubmittedScore === fixture.playerBScore;
    ...
    if (triggerProgressionPush) {
      await pushAndEmitNotification(
        req,
        [fixture.playerA, fixture.playerB],
        progressionAlert,
      );
    }
...
router.get("/:id/standings", async (req, res) => {
  try {
    ...
    const confirmedFixtures = totalFixtures.filter(
    ...
    totalFixtures.forEach((f) => {
      ...
      const idA =
        f.playerA && typeof f.playerA === "object" && f.playerA._id
          ? f.playerA._id.toString()
          : f.playerA?.toString();
      const idB =
        f.playerB && typeof f.playerB === "object" && f.playerB._id
          ? f.playerB._id.toString()
          : f.playerB?.toString();
    ...
    confirmedFixtures.forEach((match) => {
      const idA =
        match.playerA && typeof match.playerA === "object" && match.playerA._id
          ? match.playerA._id.toString()
          : match.playerA?.toString();
      const idB =
        match.playerB && typeof match.playerB === "object" && match.playerB._id
          ? match.playerB._id.toString()
          : match.playerB?.toString();
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (maxStrengthLimit !== undefined)
      league.maxStrengthLimit = maxStrengthLimit;
    if (capacity !== undefined) {
      if (capacity < 2) {
        return res
          .status(400)
          .json({ success: false, error: "Capacity must be at least 2." });
      }
      league.capacity = capacity;
    }
    if (
      status !== undefined &&
      ["recruiting", "active", "completed"].includes(status)
    ) {
    ...
    if (tournamentFormat !== undefined)
      league.tournamentFormat = tournamentFormat;
    if (groupStageCount !== undefined)
      league.groupStageCount = parseInt(groupStageCount, 10) || 4;
    ...
    if (rounds !== undefined && rounds !== null && rounds !== "") {
      ...
      if (isNaN(parsedRounds) || parsedRounds < 0) {
        return res
          ...
          .json({
            success: false,
            error: "Rounds must be a valid positive whole number.",
          });
      }
      ...
      if (parsedRounds !== league.rounds) {
        ...
        if (league.status === "active" && league.players.length > 0) {
          ...
          const formatType = String(
            league.tournamentFormat || "classic",
          ).trim();
          ...
          if (formatType === "knockout") {
            ...
            newSchedulePlan = generateClassicLeague(
              league.players,
              parsedRounds,
            );
          }
    ...
    if (league.status === "active") {
      return res
        ...
        .json({
          success: false,
          error:
            "Cannot delete an active league. Complete or cancel the tournament first.",
        });
    }
  ...
  try {
    ...
    if (league.status !== "recruiting")
      return res
        ...
        .json({
          success: false,
          error: "Cannot add players. League is already active or completed.",
        });
    if (league.players.length >= league.capacity)
      return res
        .status(400)
        .json({ success: false, error: "League capacity already reached!" });
    ...
    if (!player)
      return res
        ...
        .json({
          success: false,
          error: "Manager profile not found in system.",
        });
    if (league.players.includes(player._id))
      return res
        .status(400)
        .json({ success: false, error: "Manager is already in this league." });
...
router.get("/:id/roster", async (req, res) => {
  try {
    const league = await League.findById(req.params.id).populate(
      "players",
      "username teamStrength whatsappNumber",
    );

    if (!league) {
      return res
        .status(404)
        .json({ success: false, error: "League target not found." });
    }
...
router.post("/substitute-player", async (req, res) => {
  try {
    ...
    if (!leagueId || !oldPlayerId || !newPlayerId) {
      return res
        .status(400)
        .json({ success: false, error: "Missing required IDs." });
    }
    ...
    if (!mongoose.Types.ObjectId.isValid(newPlayerId)) {
      ...
      if (!targetUser) {
        return res
          ...
          .json({
            success: false,
            error: `Manager matching username "${newPlayerId}" not found in system.`,
          });
      }
    ...
    const playerIndex = league.players
      .map((id) => id.toString())
      .indexOf(oldPlayerId);
    if (playerIndex === -1) {
      return res
        ...
        .json({
          success: false,
          error: "Old player not found in this league's roster.",
        });
    }
...
const checkAndCompleteLeague = async (req, leagueId) => {
  try {
    ...
    const league = await League.findById(objectIdLeagueId).populate(
      "players",
      "username",
    );
    ...
    if (formatType === "knockout") {
      ...
      const currentRoundMatches = totalFixtures.filter(
        (f) => f.matchday === activeMatchday,
      );
      const confirmedMatchesInRound = currentRoundMatches.filter(
        (f) => f.status === "confirmed",
      );

      if (
        currentRoundMatches.length === 0 ||
        confirmedMatchesInRound.length !== currentRoundMatches.length
      ) {
        return;
      }
      ...
      if (
        ...
        currentRoundMatches[0].roundName === "Finals"
      ) {
        ...
        const isPlayerAWinner =
          finalMatch.playerAScore > finalMatch.playerBScore;

        // Track down the matched text username profiles securely from our populated players list
        const rawWinnerId = (
          isPlayerAWinner ? finalMatch.playerA : finalMatch.playerB
        )?.toString();
        const rawRunnerUpId = (
          isPlayerAWinner ? finalMatch.playerB : finalMatch.playerA
        )?.toString();

        const winnerUser = league.players.find(
          (p) => p._id.toString() === rawWinnerId,
        );
        const runnerUpUser = league.players.find(
          (p) => p._id.toString() === rawRunnerUpId,
        );
    ...
    if (formatType === "classic") {
      ...
      if (
        ...
        const uniqueMatchdays = [
          ...new Set(totalFixtures.map((f) => f.matchday)),
        ].sort((a, b) => a - b);
        const maxLeagueMatchday =
          uniqueMatchdays.length > 0
            ? uniqueMatchdays[uniqueMatchdays.length - 1]
            : 1;
        ...
        if (activeMatchday >= maxLeagueMatchday) {
          ...
          const allConfirmed = totalFixtures.filter(
            (f) => f.status === "confirmed",
          );
          allConfirmed.forEach((f) => {
            ...
            if (statsMap[idA] && statsMap[idB]) {
              ...
              else if (f.playerBScore > f.playerAScore)
                statsMap[idB].points += 3;
              else {
          ...
          const podiumData = {
            winner: sortedStandings[0]
              ? {
                  userId: sortedStandings[0].userId,
            ...
            runnerUp: sortedStandings[1]
              ? {
                  userId: sortedStandings[1].userId,
            ...
            thirdPlace: sortedStandings[2]
              ? {
                  userId: sortedStandings[2].userId,
    ...
    if (formatType === "group_knockout" || formatType.includes("group")) {
      ...
      const groupStageFixtures = totalFixtures.filter(
        (f) => !f.stageType || f.stageType === "group_stage",
      );
      const uniqueGroupMatchdays = [
        ...new Set(groupStageFixtures.map((f) => f.matchday)),
      ].sort((a, b) => a - b);
      const maxGroupMatchday =
        uniqueGroupMatchdays.length > 0
          ? uniqueGroupMatchdays[uniqueGroupMatchdays.length - 1]
          : 3;
      ...
      if (activeMatchday === maxGroupMatchday) {
        const playerGroupMap = {};
        totalFixtures.forEach((f) => {
          const idA = f.playerA?._id
            ? f.playerA._id.toString()
            : f.playerA?.toString();
          const idB = f.playerB?._id
            ? f.playerB._id.toString()
            : f.playerB?.toString();
          if (idA && f.groupLabel) playerGroupMap[idA] = f.groupLabel;
          if (idB && f.groupLabel) playerGroupMap[idB] = f.groupLabel;
        ...
        const standingsMap = {};
        league.players.forEach((p) => {
          standingsMap[p._id.toString()] = {
            playerId: p._id,
            username: p.username,
            groupLabel: playerGroupMap[p._id.toString()] || "A",
            points: 0,
            gd: 0,
          };
        });
        ...
        groupStageFixtures.forEach((f) => {
          if (f.status === "confirmed") {
            ...
            if (standingsMap[idA] && standingsMap[idB]) {
              ...
              if (f.playerAScore > f.playerBScore)
                standingsMap[idA].points += 3;
              else if (f.playerBScore > f.playerAScore)
                standingsMap[idB].points += 3;
              else {
        ...
        Object.values(standingsMap).forEach((row) => {
          if (!groupSegments[row.groupLabel])
            groupSegments[row.groupLabel] = [];
        ...
        const isPowerOfTwo =
          countOfSurvivors > 0 &&
          (countOfSurvivors & (countOfSurvivors - 1)) === 0;
      ...
      if (activeMatchday > maxGroupMatchday) {
        const wildcardFixtures = totalFixtures.filter(
          (f) => f.stageType === "wildcard_stage",
        );
        const isWildcardMatchdayActive =
          wildcardFixtures.length > 0 &&
          wildcardFixtures[0].matchday === activeMatchday;

        if (isWildcardMatchdayActive) {
          const wildcardWinners = currentRoundMatches.map((m) =>
            m.playerAScore > m.playerBScore ? m.playerA : m.playerB,
          );
          const playerGroupMap = {};
          totalFixtures.forEach((f) => {
            const idA = f.playerA?._id
          ...
          let roundName =
            nextMatchesCount === 1
              ...
              : nextMatchesCount === 2
                ...
                : nextMatchesCount === 4
                  ? "Quarterfinals"
                  : `Round of ${finalKnockoutPool.length}`;
        ...
        const knockoutFixtures = totalFixtures.filter(
          (f) => f.stageType === "knockout_stage",
        );
        const currentRoundKnockouts = knockoutFixtures.filter(
          (f) => f.matchday === activeMatchday,
        );

        const isFinalsMatch =
          currentRoundKnockouts.length === 1 &&
          currentRoundKnockouts[0].roundName &&
          String(currentRoundKnockouts[0].roundName).trim().toLowerCase() ===
            "finals";
    ...
    console.error(
      "Critical failure executing automated progression pipeline loops:",
      error,
    );
  }
};
...
const handleKnockoutProgressionCheck = async (req, league, totalFixtures) => {
  const activeMatchday = league.currentMatchday || 1;
  const currentMatches = totalFixtures.filter(
    (f) => f.matchday === activeMatchday,
  );
  const confirmedMatches = currentMatches.filter(
    (f) => f.status === "confirmed",
  );

  if (
    currentMatches.length === 0 ||
    confirmedMatches.length !== currentMatches.length
  )
    return;
  ...
  let advancedWinners = currentMatches.map((m) =>
    m.playerAScore > m.playerBScore ? m.playerA : m.playerB,
  );
